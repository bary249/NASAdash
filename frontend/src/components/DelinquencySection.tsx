...
export function DelinquencySection({ propertyId }: Props) {
  ...
  const totalAgingAbs = Math.abs(delinquency_aging.days_0_30) + Math.abs(delinquency_aging.days_31_60) + 
                        Math.abs(delinquency_aging.days_61_90) + Math.abs(delinquency_aging.days_90_plus);

  // Get top delinquent residents
  const topDelinquent = data.resident_details
      ...
      {evictions.unit_count > 0 && (
        <div className="bg-red-50 border border-red-200 rounded-xl p-4 mb-6">
          ...
          <div className="grid grid-cols-3 gap-4 text-center">
            ...
            <div>
              <div className="text-2xl font-bold text-red-700">{evictions.filed_count}</div>
              <div className="text-xs text-red-600">Filed</div>
            </div>
          </div>
        </div>
      )}

      {/* Top Delinquent Units - Expandable */}
        ...
        <div className="border border-slate-200 rounded-xl overflow-hidden">
          ...
          >
            <span className="text-sm font-semibold text-slate-700">
              Top Delinquent Units ({topDelinquent.length})
            </span>
            {showDetails ? (
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-sm">
                ...
                <tbody className="divide-y divide-slate-100">
                  {topDelinquent.map((resident, idx) => (
